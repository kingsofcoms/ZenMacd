              ...
              function checkHold () {
                return setTimeout(function () {
                  ...
                  syncBalance(function () {
                    var on_hold
                    if (type === 'buy') (on_hold = n(s.balance.currency_hold) > n(0))
                    else on_hold = (n(s.balance.asset_hold) > n(0))
                    if (on_hold) {
                      // wait a bit for settlement
                      msg('funds on hold after cancel, waiting 15s')
        ...
        getQuote(function (err, quote) {
          ...
          if (signal === 'buy') {
            ...
            if ((s.product.min_size && Number(size) >= Number(s.product.min_size)) || ('min_total' in s.product && s.product.min_total && n(size).multiply(price).value() >= Number(s.product.min_total))) {
              ...
              if (n(s.balance.currency).subtract(s.balance.currency_hold || 0).value() < n(size).value()) {
